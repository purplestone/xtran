...
var Cli = (function () {
  var _pub_static = function () {var _pub = {}, _pri = {}, _pro = {};
    var _init = function () {
      ...
      _pri["scriptDir"] = xtran.scriptDirDefault;
      ...
      commander.program.command('script <fileName>')
        .usage('<fileName>')
        .description('create a .js file <fileName>, some demo code in it')
        .action((cmd, oProgram) => {
          _pri.runCli('script', oProgram || cmd);
        })
      ;
    ...
    _pub["routeCmd"] = function (oCli, go) {
      switch (oCli.cmd) {
        case 'init':
          ...
          break;
        case 'script': 
          go(oCli => _pri.createScript(oCli));
          break;
        case 'list':
    ...
    _pri["shwoTranList"] = function (oCli) {
      var {extFormatDir, extModeDir} = _pri.getExtScript(oCli);
      xtran.config({
        showLoadLog: oCli.opts.loadInfo
      });
      var oTran = {
        format: xtran.getAllFormat(extFormatDir).get(),
        mode: xtran.getAllMode(extModeDir).get(),
      };
    ...
    _pri["createScript"] = function (oCli) {
      var targetFilePath = path.join(oCli.cwd, oCli.args[0]);
      var demoScriptFilePath = path.join(oCli.scriptDir, 'script.js.demo');
      var sContent = fs.readFileSync(demoScriptFilePath).toString().replace('${scriptRootDir}', oCli.scriptDir.replace(/\\/g, '/'));
      fs.writeFileSync(targetFilePath, sContent);
