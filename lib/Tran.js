...
var Tran = (function () {
	var _pub_static = function () {var _pub = {}, _pri = {}, _pro = {};
		...
		_pub["toFormat"] = function (sFormat, opt) {
			...
			return Promise.resolve(oTranFun.to(_pub.data, _pri.opt))
			...
			if (_pub.mode && _pub.mode == sMode) {
				...
				if (!_pub.mode) {
					if (_pub.format == 'json') {
						if (_pub.mode != sMode) {
							try {
								_pub.data = _pri.oModeTran[sMode].fromFormat[_pub.format](_pub.data);
							} catch (error) {}
		...
		_pub["saveFile"] = function (filePath) {
			var oFile = _pub.data;
			if ({}.toString.call(oFile) == '[object Object]' || {}.toString.call(oFile) == '[object Array]') {
				oFile = JSON.stringify(oFile);
			}
			$$fs.writeFileSync(filePath, oFile);
		};
